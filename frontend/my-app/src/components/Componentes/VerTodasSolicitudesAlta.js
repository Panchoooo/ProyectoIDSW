...
export class VerTodasSolicitudes extends React.Component{
    ...
    {  
        estado: "Dado de Alta"}
        this.UsuarioRep.verSolEsp(solicitud).then(data => {
        ...
        <React.Fragment  >
            ...
            <tbody >
                <React.Fragment >
                    ...
                    {!isLoading ? (
                    data.map(a => {
                        const {id, rut_paciente, estado,fecha_check } = a;
                        var fecha = this.fecha(fecha_check);                        
                        return (
                            <tr key={id}>
                            <th scope="row">{id}</th>
                            <td>{rut_paciente}</td>
                            <td className="text-primary" >{estado}</td>
                            <td>{fecha}</td>
